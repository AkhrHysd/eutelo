...
5. **動的パス（変数を含むパス）の処理**
   - ディレクトリパスに `{FEATURE}` などの変数が含まれる場合、`init` ではプレースホルダーパス（例: `__FEATURE__`）を作成する
   - デフォルトではプレースホルダーを作成するが、オプション（`--skip-dynamic-paths`）で無効化可能
   - `eutelo add` コマンド実行時に、実際の値で置き換えられる
   - プレースホルダーの形式は設定可能（デフォルト: `__VARIABLE__`）
...
- **動的パスの可視化**: プレースホルダーパスを作成することで、どのディレクトリが動的に生成されるかが明確
...
- **リスク**: 動的パスのプレースホルダーが実際のディレクトリと混同される可能性
  - **対策**: プレースホルダーの形式を明確にし（`__FEATURE__` など）、ドキュメントで説明する
