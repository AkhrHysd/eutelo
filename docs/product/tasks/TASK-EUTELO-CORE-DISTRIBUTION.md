...
## 2-2. 構成ガイド
- [ ] `DIRECTORY_GUIDE.md` を `/config/` に配置する
- [ ] 命名規約・配置規約が PRD/DSG と矛盾しないか確認する

## 2-3. 参考資料（例示）
- [ ] `/examples/ci/` に参考用ワークフローファイル（中身は空 or コメントのみ）を配置する
- [ ] `/examples/scaffold/` に構成例（実装なし）を配置する
...
## 5-1. セクションごとに細粒度で作成
- [ ] 「目的（Why）」を書き直す
- [ ] 「インストール / 使い方」セクションを追加する
- [ ] 「配布内容一覧（templates / config / examples）」を書き出す
- [ ] 「バージョニング方針（ADR-0106）」を記載する
- [ ] 「公開戦略（ADR-0105）」を記載する
- [ ] 「対応 Node.js（ADR-0101）」を記載する
- [ ] 「ライセンス（MIT）」を記載する
