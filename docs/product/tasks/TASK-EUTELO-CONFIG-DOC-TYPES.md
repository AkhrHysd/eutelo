...
- [ ] `DocumentTypeRegistry.getDocumentTypes()` が Config から DocumentType 一覧を返すテスト
- [ ] `DocumentTypeRegistry.hasDocumentType()` が正しく判定するテスト
- [ ] `DocumentTypeRegistry.getScaffoldConfig()` が scaffold エントリを返すテスト
- [ ] `DocumentTypeRegistry.getFrontmatterSchema()` が schema を返すテスト
- [ ] 未登録 DocumentType に対して `undefined` を返すテスト
- 期待失敗内容: DocumentTypeRegistry クラスが存在しない / Config から DocumentType を抽出できない

#### Phase 2: AddDocumentService の変更テスト

- [ ] `getBlueprint()` が Config から scaffold エントリを検索するテスト
- [ ] `kind` でマッチングするテスト
- [ ] `scaffoldId` で直接指定するテスト
- [ ] 未登録 DocumentType に対して `DocumentTypeNotFoundError` をスローするテスト
- [ ] エラーメッセージに利用可能な DocumentType 一覧が含まれるテスト
...
- [ ] `packages/core/src/config/DocumentTypeRegistry.ts` を作成
- [ ] `EuteloConfigResolved` から DocumentType 一覧を抽出するロジックを実装
- [ ] `scaffold` エントリの `kind` を DocumentType として扱う
- [ ] `getDocumentTypes()`, `hasDocumentType()`, `getScaffoldConfig()`, `getFrontmatterSchema()` メソッドを実装
- [ ] ConfigResolver に統合（または独立したサービスとして提供）

#### Phase 2: AddDocumentService の変更

- [ ] `getBlueprint()` メソッドを変更して Config から scaffold エントリを検索
- [ ] `kind` または `scaffoldId` でマッチングするロジックを実装
- [ ] `DocumentTypeNotFoundError` クラスを作成
- [ ] エラーメッセージに利用可能な DocumentType 一覧を含める
- [ ] 既存のテストが引き続き成功することを確認

#### Phase 3: CLI コマンド動的生成の実装

- [ ] `packages/cli/src/index.ts` の `add` コマンド定義を変更
- [ ] Config 解決後に scaffold エントリを走査してサブコマンドを動的生成
- [ ] 既存の固定コマンド（`prd`, `beh`, `dsg` など）との互換性を維持
- [ ] `--help` で利用可能な種別が表示されるようにする
- [ ] エラーハンドリングを追加（未登録 DocumentType の場合）

#### Phase 4: ValidationService の変更

- [ ] `packages/core/src/services/ValidationService.ts` を変更
- [ ] Config から解決された DocumentType のみを許可するロジックを追加
- [ ] 未登録 DocumentType に対して警告を出力するロジックを追加
- [ ] 警告のみで exit code が 0 のままになるようにする

#### Phase 5: GraphService の変更

- [ ] `packages/core/src/graph/DocumentScanner.ts` を変更
- [ ] Config から解決された DocumentType のみを Graph に含めるロジックを追加
- [ ] 未登録 DocumentType のドキュメントを警告として記録するロジックを追加

#### Phase 6: エラーハンドリングの改善

- [ ] `DocumentTypeNotFoundError` のメッセージを改善
- [ ] CLI 側でエラーメッセージを適切に表示
- [ ] 利用可能な DocumentType 一覧を表示するヘルパー関数を追加

#### Phase 7: ドキュメント更新

- [ ] `README.md` の `eutelo add` セクションにカスタムDocumentTypeの説明を追加
  - [ ] カスタムDocumentTypeの追加方法を説明
  - [ ] `eutelo add <custom-type>` の使用例を追加
  - [ ] Config で定義された DocumentType が自動的に CLI コマンドとして利用可能になることを説明
- [ ] `README.jp.md` の `eutelo add` セクションに同様の内容を日本語で追加
- [ ] Config ドキュメント（`docs/` 配下）に DocumentType 追加手順を詳細に記載
  - [ ] テンプレートファイルの作成方法
  - [ ] `eutelo.config.ts` での scaffold エントリの定義方法
  - [ ] frontmatter schema の定義方法
  - [ ] 3ステップ以内での追加手順を明記
- [ ] `docs/DIRECTORY_GUIDE.md` を更新（必要に応じて）
  - [ ] カスタムDocumentTypeのディレクトリ配置例を追加
- [ ] `docs/README.md` を更新（必要に応じて）
  - [ ] DocumentType拡張機能への言及を追加
- [ ] CHANGELOG.md に変更内容を記載
  - [ ] DocumentType拡張機能の追加を記載
  - [ ] 後方互換性の維持を明記
